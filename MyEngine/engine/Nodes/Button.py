...
class ButtonNode(UINode):
    ...
    def draw(self, screen):
        self.button_rect = pygame.draw.rect(screen, self.color, (self.position[0], self.position[1], self.width, self.height))
        
        my_label = self.my_font.render(self.text, 1, self.text_color)
        
        match self.text_align:
            case "Center":
                text_rect = my_label.get_rect(center=(self.position[0] + self.width/2, self.position[1] + self.height/2))
                screen.blit(my_label, text_rect)
            case "Left":
                screen.blit(my_label, (self.position[0], self.position[1]))
            case "Right":
                text_rect = my_label.get_rect(right=self.position[0] + self.width, centery=self.position[1] + self.height/2)
                screen.blit(my_label, text_rect)
